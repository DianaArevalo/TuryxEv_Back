...
export class HotelEdit {
    ...
    async handler(props: HotelEditHandlerProps){
        ...
        //cambio ubicacion 
        if (props.location) 
            hotel.location = (await HotelLocation.create(props.location)
        ) as HotelLocation;

        //nombre, cambiar nombre
        if (props.name && props.name !== hotel.name.getValue()) 
        ...
        if (props.password) {
            if (hotel.password?.getValue() === "AUTH") {
                throw new ValidationError(
                    "Can't update password when you sign in with an external provider"
                );
            }
            if (props.password !== hotel.password?.getValue()) {
        ...
        if (props.status && props.status !== hotel.status.getValue()) 
            hotel.status = HotelStatus.create(props.status as HotelStatusT) as HotelStatus;

        //score, cambiar score
        if (props.score && props.score !== hotel.score.getValue().toString()) 
